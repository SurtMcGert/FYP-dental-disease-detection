...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task):
        ...
        pred_results = []
        pbar = tqdm(
            dataloader, desc=f"Inferencing model in {task} datasets.", ncols=NCOLS)
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            if self.do_pr_metric:
                import copy
                eval_outputs = copy.deepcopy(
                    [x.detach().cpu() for x in outputs])
            ...
            for si, pred in enumerate(eval_outputs):
                ...
                # native-space pred
                self.scale_coords(
                    imgs[si].shape[1:], predn[:, :4], shapes[si][0], shapes[si][1])
                ...
                if nl:
                    ...
                    # native-space labels
                    self.scale_coords(imgs[si].shape[1:],
                                      tbox, shapes[si][0], shapes[si][1])
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                AP50_F1_max_idx = len(f1.mean(0)) - \
                    f1.mean(0)[::-1].argmax() - 1
                LOGGER.info(
                    f"IOU 50 best mF1 thershold near {AP50_F1_max_idx/1000.0}.")
                ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
                mp, mr, map50, map = p[:, AP50_F1_max_idx].mean(
                ), r[:, AP50_F1_max_idx].mean(), ap50.mean(), ap.mean()
                ...
                LOGGER.info(pf % ('all', seen, nt.sum(), mp, mr,
                            f1.mean(0)[AP50_F1_max_idx], map50, map))
                ...
                if self.plot_confusion_matrix:
                    confusion_matrix.plot(
                        save_dir=self.save_dir, names=list(model.names))
            else:
        ...
        if task != 'speed' and len(pred_results):
            if 'anno_path' in self.data:
                ...
                dataset_root = os.path.dirname(
                    os.path.dirname(self.data[task][0]))
                base_name = os.path.basename(self.data[task][0])
                anno_json = os.path.join(
                    dataset_root, 'annotations', f'instances_{base_name}.json')
            ...
            if self.verbose:
                ...
                label_count_dicts = [copy.deepcopy(
                    label_count_dict) for _ in range(model.nc)]
                for _, ann_i in cocoEval.cocoGt.anns.items():
                    if ann_i["ignore"]:
                        continue
                    val_dataset_anns_count += 1
                    nc_i = self.coco80_to_coco91_class().index(
                        ann_i['category_id']) if self.is_coco else ann_i['category_id']
                    label_count_dicts[nc_i]["images"].add(ann_i["image_id"])
                    label_count_dicts[nc_i]["anns"] += 1

                s = ('%-16s' + '%12s' * 7) % ('Class', 'Labeled_images', 'Labels',
                                              'P@.5iou', 'R@.5iou', 'F1@.5iou', 'mAP@.5', 'mAP@.5:.95')
                LOGGER.info(s)
                ...
                mp = np.array([np.mean(coco_p_iou50[ii][coco_p_iou50[ii] > -1])
                              for ii in range(coco_p_iou50.shape[0])])
                mr = np.linspace(.0, 1.00, int(
                ...
                for nc_i in range(model.nc):
                    ...
                    p = coco_p_c_iou50
                    r = np.linspace(.0, 1.00, int(
                        np.round((1.00 - .0) / .01)) + 1, endpoint=True)
                    f1 = 2 * p * r / (p + r + 1e-16)
                    i = f1.argmax()
                    LOGGER.info(pf % (model.names[nc_i], len(
                        label_count_dicts[nc_i]["images"]), label_count_dicts[nc_i]["anns"], p[i], r[i], f1[i], map50, map))
            cocoEval.summarize()
    ...
    def eval_speed(self, task):
        ...
        if task != 'train':
            ...
            pre_time, inf_time, nms_time = 1000 * \
                self.speed_result[1:].cpu().numpy() / n_samples
    ...
    def scale_coords(self, img1_shape, coords, img0_shape, ratio_pad=None):
        ...
        if isinstance(coords, torch.Tensor):  # faster individually
            ...
            coords[:, [0, 2]] = coords[:, [0, 2]].clip(
                0, img0_shape[1])  # x1, x2
            coords[:, [1, 3]] = coords[:, [1, 3]].clip(
    ...
    def convert_to_coco_format(self, outputs, imgs, paths, shapes, ids):
        ...
        for i, pred in enumerate(outputs):
            ...
            self.scale_coords(imgs[i].shape[1:],
                              pred[:, :4], shape, shapes[i][1])
    ...
    def eval_trt(self, engine, stride=32):
        ...
        pred_results = []
        pbar = tqdm(
            dataloader, desc="Inferencing model in validation dataset.", ncols=NCOLS)
        for imgs, targets, paths, shapes in pbar:
            ...
            pred_results.extend(convert_to_coco_format_trt(
                nums, boxes, scores, classes, paths, shapes, self.ids))
