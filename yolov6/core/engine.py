...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        if hasattr(self, "ckpt"):
            resume_state_dict = self.ckpt['model'].float(
            ).state_dict()  # checkpoint state_dict as FP32
            ...
            if self.main_process:
                self.ema.ema.load_state_dict(
                    self.ckpt['ema'].float().state_dict())
    ...
    def train_in_steps(self, epoch_num, step_num):
        ...
        if self.write_trainbatch_tb and self.main_process and self.step == 0:
            ...
            write_tbimg(self.tblogger, self.vis_train_batch,
                        self.step + self.max_stepnum * self.epoch, type='train')
    ...
    def after_epoch(self):
        ...
        if self.main_process:
            ...
            eval_interval = self.args.eval_interval if remaining_epochs >= self.args.heavy_eval_range else min(
                3, self.args.eval_interval)
            ...
            save_checkpoint(ckpt, (is_val_epoch) and (
                self.ap == self.best_ap), save_ckpt_dir, model_name='last_ckpt')
            if self.epoch >= self.max_epoch - self.args.save_ckpt_on_last_n_epoch:
                save_checkpoint(ckpt, False, save_ckpt_dir,
            ...
            write_tbimg(self.tblogger, self.vis_imgs_list,
                        self.epoch, type='val')
        ...
        if not hasattr(self.cfg, "eval_params"):
            ...
            results, vis_outputs, vis_paths = eval.run(self.data_dict,
                                                       batch_size=get_cfg_value(
                                                           self.cfg.eval_params, "batch_size", self.batch_size // self.world_size * 2),
                                                       img_size=eval_img_size,
                                                       model=self.ema.ema if self.args.calib is False else self.model,
                                                       conf_thres=get_cfg_value(
                                                           self.cfg.eval_params, "conf_thres", 0.03),
                                                       dataloader=self.val_loader,
                                                       save_dir=self.save_dir,
                                                       task='train',
                                                       shrink_size=get_cfg_value(
                                                           self.cfg.eval_params, "shrink_size", eval_img_size),
                                                       infer_on_rect=get_cfg_value(
                                                           self.cfg.eval_params, "infer_on_rect", False),
                                                       verbose=get_cfg_value(
                                                           self.cfg.eval_params, "verbose", False),
                                                       do_coco_metric=get_cfg_value(
                                                           self.cfg.eval_params, "do_coco_metric", True),
                                                       do_pr_metric=get_cfg_value(
                                                           self.cfg.eval_params, "do_pr_metric", False),
                                                       plot_curve=get_cfg_value(
    ...
    def before_train_loop(self):
        ...
        self.warmup_stepnum = max(round(
            self.cfg.solver.warmup_epochs * self.max_stepnum), 1000) if self.args.quant is False else 0
    ...
    def before_epoch(self):
        ...
        if self.epoch == self.max_epoch - self.args.stop_aug_last_n_epoch:
            self.cfg.data_aug.mosaic = 0.0
            self.cfg.data_aug.mixup = 0.0
            # disable cache ram when stop strong augmentation.
    ...
    def update_optimizer(self):
        ...
        if curr_step <= self.warmup_stepnum:
            self.accumulate = max(1, np.interp(curr_step, [0, self.warmup_stepnum], [
    ...
    def get_data_loader(args, cfg, data_dict):
        ...
        assert len(
            class_names) == nc, f'the length of class names does not match the number of classes defined'
    ...
    def get_model(self, args, cfg, nc, device):
        if 'YOLOv6-lite' in cfg.model.type:
            ...
            model = build_model(
                cfg, nc, device, fuse_ab=self.args.fuse_ab, distill_ns=self.distill_ns)
        weights = cfg.model.pretrained
        if weights:  # finetune if pretrained model is set
            if not os.path.exists(weights):
                download_ckpt(weights)
            LOGGER.info(
                f'Loading state_dict from {weights} for fine-tuning...')
    ...
    def plot_train_batch(self, images, targets, max_size=1920, max_subplots=16):
        ...
        for i in range(bs):
            x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
            cv2.rectangle(mosaic, (x, y), (x + w, y + h),
                          (255, 255, 255), thickness=2)  # borders
            ...
            if len(targets) > 0:
                ...
                for j, box in enumerate(boxes.T.tolist()):
                    ...
                    if labels:
                        label = f'{cls}'
                        cv2.rectangle(
                            mosaic, (box[0], box[1]), (box[2], box[3]), color, thickness=1)
                        cv2.putText(
        ...
        for (vis_output, vis_path) in zip(vis_outputs, vis_paths):
            ...
            for bbox_idx, vis_bbox in enumerate(vis_output_array):
                ...
                if box_score < vis_conf or bbox_idx > vis_max_box_num:
                    break
                cv2.rectangle(ori_img, (x_tl, y_tl), (x_br, y_br), tuple(
                    [int(x) for x in self.color[cls_id]]), thickness=1)
                cv2.putText(ori_img, f"{self.data_dict['names'][cls_id]}: {box_score:.2f}", (
                    x_tl, y_tl - 10), cv2.FONT_HERSHEY_COMPLEX, 0.5, tuple([int(x) for x in self.color[cls_id]]), thickness=1)
            self.vis_imgs_list.append(
                torch.from_numpy(ori_img[:, :, ::-1].copy()))
    ...
    def quant_setup(self, model, cfg, device):
        if self.args.quant:
            ...
            model.neck.upsample_enable_quant(
                cfg.ptq.num_bits, cfg.ptq.calib_method)
            # if self.main_process:
            #     print(model)
            # QAT
            if self.args.calib is False:
                ...
                model.load_state_dict(torch.load(cfg.qat.calib_pt)[
